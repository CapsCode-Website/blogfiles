...
The above code will run perfectly fine and body will be able to give background of black.
...
Now the twist comes here is,
if we define --fontsize: 50px in the :root inside media query, then it will not work as expected for the screen size bigger than 450px.
```css
@media screen and (min-width: 450px) {
  :root {
    --blue: lightblue;
...
```
because for container class we have defined the --fontsize as local variable, it will use the local variable --fontsize and we know that 
**local variable will override global**
if we reassign the --fontsize variable with different value then it should be within container class only as we have seen above.
...
Thank you for reading this so far. This is a brief introduction on **Variables In CSS** and different aspects of it.

You can checkout some of examples in our [codesandbox](https://codesandbox.io/s/example2-nqx5qm?file=/index.css).
